...
def arrays_for_npz(
    ...
    """
    Build NPZ arrays:
    Modified for MotionGPT/HumanML3D pipeline compatibility.
    """
    ...
    #===========================================================
    # 关节位置 (Positions)
    # 源数据 key: 'global_translation'
    # 形状: (T, J, 3) -> 对应 HumanML3D 需要的 pos
    if "global_translation" in sample:
        out["pos"] = sample["global_translation"] 

    # 关节旋转 (Rotations)
    # 源数据 key: 'global_rotation_quat' (四元数)
    # 形状: (T, J, 4) -> 后续我们可以直接用这个算 6D rot，不用跑 IK
    if "global_rotation_quat" in sample:
        out["rot"] = sample["global_rotation_quat"]

    # 根节点信息 (可选，但保留着有用)
    if "global_root_translation" in sample:
        out["root_pos"] = sample["global_root_translation"]
    if "global_root_rotation_quat" in sample:
        out["root_rot"] = sample["global_root_rotation_quat"]
        
    # 关节速度 (可选，HumanML3D 一般自己算，但留着参照也不错)
    if "global_velocity" in sample:
        out["vel"] = sample["global_velocity"]
